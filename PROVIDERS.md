...
> **前端多选批量**（账户面板「签到选中 / 当前列表签到」）不再走 `/batch` 端点，改为前端逐账户调用单签接口并实时更新进度：同样按设置跳过（已签/禁用/重试关闭/每日上限）并打乱顺序、随机间隔。逐账户请求天然低于反向代理（Cloudflare 等）的源站等待上限，避免整批被掐断后前端卡在 0/N。`/batch` 端点仍保留，签到记录面板「失败账户重试」继续使用。
